...
class CartPoleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Steering with different force magnitude basing on action
        if action == 0:
            force = -10.0
        if action == 1:
            force = -5.0
...
def cartpole():
    ...
    while True:
        if run_after_save == SAVE_THRESHOLD:
            dqn_solver.model.save("Raw models/moderate/model" + str(run))
            pickle.dump(dqn_solver.memory, open("Raw models/moderate/model" + str(run) + ".mem", "wb"))
        ...
        if episode < EPISODE_MAX:
            ...
            episode_reward += step
            if episode == EPISODE_MAX:
                scores.append(episode_reward / EPISODE_MAX)
                epochs.append(run)
                episode = 0
                episode_reward = 0
                ...
                if episode_reward / EPISODE_MAX > POINTNTS_TO_COMPLETE:
                    dqn_solver.save("Trained models/moderate_final")
