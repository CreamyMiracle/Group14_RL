...
def cartpole():
    ...
    while True:
        if run_after_save == SAVE_THRESHOLD:
            dqn_solver.model.save("Raw models/simple/model" + str(run))
            pickle.dump(dqn_solver.memory, open("Raw models/simple/model" + str(run) + ".mem", "wb"))
        ...
        if episode < EPISODE_MAX:
            ...
            if episode == EPISODE_MAX:
                ...
                if episode_reward / EPISODE_MAX > POINTNTS_TO_COMPLETE:
                    POINTNTS_TO_COMPLETE = episode_reward / EPISODE_MAX
                    dqn_solver.save("Trained models/simple_final")
