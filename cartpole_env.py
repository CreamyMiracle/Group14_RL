# -*- coding: utf-8 -*-
"""
Created on Thu Oct 29 15:45:40 2020
...
class CartPoleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.kinematics_integrator == 'euler':
            x = x + self.tau * x_dot
            x_dot = x_dot + self.tau * x_dot_dot
            theta = theta + self.tau * theta_dot
            theta_dot = theta_dot + self.tau * theta_dot_dot
    ...
    def pot_energy(self):
        if self.state is not None:
            ...
            height = np.cos(theta_diff) * self.length
            corr_height = height + self.length
